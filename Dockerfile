...
RUN apt-get update -qq && DEBIAN_FRONTEND=noninteractive DEBCONF_NONINTERACTIVE_SEEN=true apt-get install -q -y apache2 mysql-client mysql-server libdbd-mysql-perl libjson-perl subversion openbsd-inetd git
...
RUN ln -s /www/rforge/etc/apache2/sites-available/rforge.conf /etc/apache2/sites-available/rforge.conf && \
  a2enmod rewrite && a2enmod cgi && a2dissite 000-default && \
